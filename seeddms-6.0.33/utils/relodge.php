...
function usage() { /* {{{ */
	echo "Usage:".PHP_EOL;
	echo "  seeddms-relodge [--config <file>] [-u <user>] [-h] [-v] [-t] [-q] [-o] [-f <email>] [-u <user>] [-w] [-b <base>] [-c] -d <days> -D <days>".PHP_EOL;
	echo PHP_EOL;
...
foreach($users as $user) {
	...
	$queryStr = "SELECT DISTINCT a.*, `tblDocumentContent`.`date` as contentdate, `tblDocumentStatusLog`.*, `tblDocumentAttributes`.* FROM `tblDocuments` a ".
		...
		"LEFT JOIN `tblDocumentAttributes` ON a.`id`=`tblDocumentAttributes`.`document` ".
		...
		") AND `tblDocumentAttributes`.`attrdef` IN (".implode(',', $attrids).") ";
	if(!$showobsolete)
