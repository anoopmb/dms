...
require_once("inc.ClassStorage.php");
require_once("inc.ClassStorageFile.php");

/**
 * Class to represent the complete document management system.
 * This class is needed to do most of the dms operations. It needs
 * an instance of {@see SeedDMS_Core_DatabaseAccess} to access the
 ...
 * a regular file system starting with a {@see SeedDMS_Core_DMS::rootFolderID}
 *
 ...
 * {@see SeedDMS_Core_Folder::getAccessMode()} and
 * {@see SeedDMS_Core_Document::getAccessMode()} and interpret them as desired.
 ...
 * {@see SeedDMS_Core_DMS::filterAccess()}
 * and {@see SeedDMS_Core_DMS::filterUsersByAccess()}
 *
 * Though, this class has a method to set the currently logged in user
 * ({@see SeedDMS_Core_DMS::setUser()}), it does not have to be called, because
 * there is currently no class within the SeedDMS core which needs the logged
 * in user. {@see SeedDMS_Core_DMS} itself does not do any user authentication.
...
class SeedDMS_Core_DMS {
	...
	/**
	 * @var SeedDMS_Core_Storage $storage reference to storage object.
	 * This must be an instance {@see SeedDMS_Core_Storage_File}.
	 * @access protected
	 */
	protected $storage;

	/**
	 * @var object $memcache reference to memcache.
	 * @access protected
	 */
	public $memcache;
	 ...
	 *      an instance of {@see SeedDMS_Core_User}. This variable is currently not
	 *      used. It is set by {@see SeedDMS_Core_DMS::setUser()}.
	 * @access private
	 */
	private $user;
	...
	/**
	 * @var boolean $forceLink use linkFile() instead of copyFile() when
	 *      copying the document content into the data store. The default is
	 *      to copy the file. This parameter only affects the method
	 *      SeedDMS_Core_Document::addDocument(). Use this with care,
	 *      because it will leave the original document at its place.
	 * @access public
	 */
	public $forceLink;
	 ...
	 * {@see SeedDMS_Core_DMS::setCallback()}.
...
//	public $_dms;
	 ...
	 *
	 * This method is only applicable on list containing objects which have
	 ...
	 * two objects, which often isn't required. The method will first check
	 ...
	 * Hence, passing a group instead of a user is possible.
	 *
	 ...
	 *
	 * @param SeedDMS_Core_DatabaseAccess $db object of class {@see SeedDMS_Core_DatabaseAccess}
	...
	/**
	 * Return storage where files are stored
	 *
	 * This method returns the storage object as it was set by the second
	 * parameter of the constructor.
	 *
	 ...
	 * Set memcache server
	 *
	 * This method must be called right after creating an instance of
	 * {@see SeedDMS_Core_DMS}
	 *
	 * If the memcache server is set, SeedDMS_Core_DMS will make use of
	 * it if possible.
	 *
	 ...
	 *
	 * Returns the currently logged in user, previously set by {@see SeedDMS_Core_DMS::setUser()}
	 *
		...
		if (is_int($date) || is_string($date)) {
			...
			if (!empty($date['start'])) {
				if (is_int($date['start']))
					...
					$tmp = explode('-', $date['start'], 3);
					if (count($tmp) != 3)
						return false;
					if (!self::checkDate($date, 'Y-m-d'))
						return false;
					$startts = mktime(0, 0, 0, (int) $tmp[1], (int) $tmp[2], (int) $tmp[0]);
				}
			} else {
				$startts = time();
			}
	 ...
	 *
	 * This method searches a document by its name and restricts the search
	 ...
	 *
	 * This method searches a document by the name of the last document
	...
	public function countTasks($listtype, $user = null, $param5 = true) { /* {{{ */
		...
		case 'SleepingReviseByMe': // Documents I have to receipt {{{
			...
			$queryStr .= "AND `tblDocumentContent`.`revisiondate` IS NOT NULL AND `tblDocumentContent`.`revisiondate` <= ".$this->db->getCurrentDatetime(14*86400)." ";
...
//		echo $queryStr;
		...
		case 'SleepingReviseByMe': // Documents I have to revise but are still sleeping {{{
			...
			if ($groups)
				$queryStr .= "OR `tblDocumentRevisors`.`type` = 1 AND `tblDocumentRevisors`.`required` IN (".implode(',', $groups).")";
			$queryStr .= ") ";
			$queryStr .= "AND `tblDocumentContent`.`revisiondate` IS NOT NULL AND `tblDocumentContent`.`revisiondate` <= ".$this->db->getCurrentDatetime($dayoffset*86400)." ";
			$queryStr .= "AND `tblDocumentStatusLog`.`status` = ".S_RELEASED." ";
			$queryStr .= " AND `tblDocumentRevisionLog`.`status` = -3 ";
			if ($orderby == 'e') $queryStr .= "ORDER BY `expires`";
			else if ($orderby == 'u') $queryStr .= "ORDER BY `statusDate`";
		...
		case 'DueRevision': // Documents with a due revision, which is not started {{{
			...
			$queryStr .= "WHERE `tblDocumentContent`.`revisiondate` IS NOT NULL AND `tblDocumentContent`.`revisiondate` <= ".$this->db->getCurrentDatetime($dayoffset*86400)." ";
			$queryStr .= "AND `tblDocumentStatusLog`.`status` = ".S_RELEASED." ";
			if ($orderby == 'e') $queryStr .= "ORDER BY `expires`";
			else if ($orderby == 'u') $queryStr .= "ORDER BY `statusDate`";
...
//				"ORDER BY `statusDate` DESC";
	 ...
	 *
	 * This method is much like `mktime()` but does some range checks
	...
	public function search($query, $limit = 0, $offset = 0, $logicalmode = 'AND', $searchin = array(), $startFolder = null, $owner = null, $status = array(), $creationstartdate = array(), $creationenddate = array(), $modificationstartdate = array(), $modificationenddate = array(), $categories = array(), $attributes = array(), $mode = 0x3, $expirationstartdate = array(), $expirationenddate = array(), $reception = array()) { /* {{{ */
		...
		$mimetype = '';
		if (is_array($query)) {
			foreach (array('limit', 'offset', 'logicalmode', 'searchin', 'startFolder', 'owner', 'status', 'mimetype', 'creationstartdate', 'creationenddate', 'modificationstartdate', 'modificationenddate', 'categories', 'attributes', 'mode', 'revisionstartdate', 'revisionenddate', 'expirationstartdate', 'expirationenddate', 'reception') as $paramname)
		...
		if ($mode & 0x1) {
			...
			if ($revisionstartdate) {
				if (is_numeric($revisionstartdate))
					$startdate = date('Y-m-d', $revisionstartdate);
				else
			...
			if ($revisionenddate) {
				if (is_numeric($revisionenddate))
					$startdate = date('Y-m-d', $revisionenddate);
				else
...
//				"`ttreceiptid`.`maxLogID`=`tblDocumentReceiptLog`.`receiptLogID` AND ".
			...
			// mime type
			if ($mimetype) {
				$searchQuery .= " AND `tblDocumentContent`.`mimeType` IN ('".implode("','", $mimetype)."')";
			}
	...
	public function getFolder($id) { /* {{{ */
		...
		$classname = $this->classnames['folder'];
		$folder = $classname::getInstance($id, $this);
		if ($this->usecache)
			$this->cache['folders'][$id] = $folder;
		return $folder;
	} /* }}} */
	 ...
	 *
	 * This method retrieves a folder from the database by its name. The
	 ...
	 *
	 * This method retrieves a user from the database by its login.
	 ...
	 *
	 * This method retrieves a user from the database by its email.
	 ...
	 * Add a new user
	 *
	 * This method calls the hook `onPostAddUser` after the user has been
	 * added successfully.
	 *
	 * @param string $login login name
	 * @param string $pwd hashed password of new user
	 ...
	 *
	 * This method calls the hook `onPostAddDocumentCategory` if the new
	 ...
	 *
	 * deprecated: User {@see SeedDMS_Core_Group::getNotifications()}
	 *
	 ...
	 *
	 * deprecated: User {@see SeedDMS_Core_User::getNotifications()}
	 *
	 ...
	 *
	 * This method searches a previously created password request and
	 * returns the user.
	 *
	 * @param string $hash
	 ...
	 *
	 * This method retrieves a attribute definitionr from the database by
	 * its id.
	 *
	...
	public function getAttributeDefinition($id) { /* {{{ */
		...
		$attrdef = new SeedDMS_Core_AttributeDefinition($resArr["id"], $resArr["name"], (int) $resArr["objtype"], (int) $resArr["type"], $resArr["multiple"], $resArr["minvalues"], $resArr["maxvalues"], $resArr["valueset"], $resArr["regex"]);
		$attrdef->setDMS($this);
		return $attrdef;
	} /* }}} */
	 ...
	 *
	 * @param integer|array $objtype select those attribute definitions defined for an object type
	 * @param integer|array $type select those attribute definitions defined for a type
	...
	public function getAllAttributeDefinitions($objtype = 0, $type = 0) { /* {{{ */
		...
		for ($i = 0; $i < count($resArr); $i++) {
			$attrdef = new SeedDMS_Core_AttributeDefinition($resArr[$i]["id"], $resArr[$i]["name"], (int) $resArr[$i]["objtype"], (int) $resArr[$i]["type"], $resArr[$i]["multiple"], $resArr[$i]["minvalues"], $resArr[$i]["maxvalues"], $resArr[$i]["valueset"], $resArr[$i]["regex"]);
			$attrdef->setDMS($this);
			$attrdefs[$i] = $attrdef;
		}
	 ...
	 *
	 * This function retrieves all waiting receptions and it associated
	 * document version
	 *
	...
	/**
	 * Returns documents which have link to themselves
	 *
	 * @return array|bool
	 */
	...
	public function getStatisticalData($type = '') { /* {{{ */
		switch ($type) {
			...
			case 'docsperstatus':
				...
				$queryStr = "SELECT b.`status` AS `key`, count(b.`status`) AS total, b.`status` AS `res` FROM (SELECT a.`id`, max(c.`statusLogID`) AS maxlog FROM `tblDocuments` a LEFT JOIN `tblDocumentStatus` b ON a.id=b.`documentID` LEFT JOIN `tblDocumentStatusLog` c ON b.`statusID`=c.`statusID` GROUP BY a.`id` ORDER BY a.id) a LEFT JOIN `tblDocumentStatusLog` b ON a.maxlog=b.`statusLogID` GROUP BY b.`status`";
				$resArr = $this->db->getResultArray($queryStr);
				if (is_bool($resArr) && $resArr == false)
					return false;
			...
			case 'docsaccumulated':
				...
				foreach ($resArr as &$res) {
					 ...
					 * a key '`key`'
					 */
					$res['key'] = mktime(12, 0, 0, (int) substr($res['key'], 5, 2), (int) substr($res['key'], 8, 2), (int) substr($res['key'], 0, 4)) * 1000;
					$res['total'] = $sum;
				}
				return $resArr;
			case 'docstotal':
	...
	public function getMimeTypes() { /* {{{ */
		$queryStr = "SELECT `mimeType`, COUNT(`mimeType`) AS `c` FROM `tblDocumentContent` GROUP BY `mimeType` ORDER BY `mimeType`";
	 ...
	 *
	 * Setting a callback with this method will remove all previously
	 * set callbacks. Use {@see SeedDMS_Core_DMS::addCallback()} to register
	 * additional callbacks.
	 * This method does not check if there is a callback with the given name.
	 *
